...
def getEmissionsWithFullDescription():
    ...
    #for e in up_emissions['data']['results']:
    #    if not(e["title"] == "0"):
    #        emissions.append( e )

    full_emissions = {}
    full_emissions[ "Emissions" ] = sorted(  up_emissions['results'], key=lambda e: e[ "title" ].lower() ) #sorted(up_emissions['results'], key=lambda emission: emission.title)

    #Container.SetSortMethod(xbmcplugin.SORT_METHOD_LABEL_IGNORE_THE)
